...
class MessageTest extends AbstractTest
{
    ...
    public function testEmailAddress()
    {
        ...
        $this->assertInstanceOf('\Wisesight\Imap\Message\EmailAddress', $from);
        $this->assertEquals('no_host', $from->getMailbox());

        $cc = $message->getCc();
        $this->assertCount(2, $cc);
        $this->assertInstanceOf('\Wisesight\Imap\Message\EmailAddress', $cc[0]);
        $this->assertEquals('This one is right', $cc[0]->getName());
        $this->assertEquals('ding@dong.com', $cc[0]->getAddress());
        
        $this->assertInstanceOf('\Wisesight\Imap\Message\EmailAddress', $cc[1]);
